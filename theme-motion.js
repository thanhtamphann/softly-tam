(()=>{
  ...
  function reconcile(){removeLegacy();applyMode(currentMode());installToggle();installMotion()}
  async function init(){const control=await loadControl();const design=control.design||{};allowToggle=design.allowVisitorDarkMode!==false;defaultMode=design.defaultMode==='dark'?'dark':'light';reconcile();[650,1200,2200].forEach(ms=>setTimeout(reconcile,ms));window.addEventListener('storage',e=>{if(e.key===STORAGE_KEY||e.key===OLD_KEY)reconcile()})}
